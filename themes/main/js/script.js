...
var app = {
    ...
    setup: {
        ...
        newspage: function() {
            var newsCard = $('.nws-frame2 .nws-wrapper > a');
            for (var i = 0; i < newsCard.length; i += 5) {
                newsCard.slice(i, i + 5).wrapAll('<div class="nws-group"></div>');
            }
        },

        actionpage: function() {
            const swiper = new Swiper('.action-frame4__slider', {
                // Optional parameters
                slidesPerView: 1,
                spaceBetween: 20,
                centeredSlides: true,
                loop: true,
                navigation: {
                    nextEl: '.action-frame4 .arrow-right',
                },
                breakpoints: {
                    768: {
                        slidesPerView: 2,
